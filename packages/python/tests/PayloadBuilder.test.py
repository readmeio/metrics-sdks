...
class PalyloadBuilderTestCase(unittest.TestCase):
    #Unit Tests for Payload Builder to make sure the created payloads are created correctly to the json request
    #Requests data can either be grabbed from an actual request, or maybe fake some json data that's similar in datatype as the one PayloadBuilder receives
    ...
    def createPayload(self, config):
        ...
        return PayloadBuilder(config.BLACKLIST, config.WHITELIST, config.IS_DEVELOPMENT_MODE, config.GROUPING_FUNCTION)
    ...
    #for test GET/POST/PUT I'm putting the status code tests for now since we cant verify the body yet
    #for status 401 and 403, they can also be moved to blacklisted(?)
