...
import static com.readme.config.ReadmeApiConfig.README_API_URL;
import static com.readme.datatransfer.ReadmeApiKeyEncoder.encode;
...
public class BaseLogUrlFetcher {
    ...
    public static String fetchBaseLogUrl(String readmeApiKey) {
        Function<String, String> fetcher = key
                -> baseUrlCache.get(key, BaseLogUrlFetcher::fetchBaseUrlFromApi);
        return fetcher.apply(readmeApiKey);
    }

    private static String fetchBaseUrlFromApi(String readmeApiKey) {
        String encodedApiKey = encode(readmeApiKey);

        Request request = new Request.Builder()
                .url(README_API_URL + "/v1")
