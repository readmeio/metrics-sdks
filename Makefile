...
test-metrics-dotnet: ## Run Metrics tests against the .NET SDK
	...
	README_ALLOWLIST=true docker-compose up --build --detach integration_dotnet_metrics_v6.0
	sleep 5
	README_ALLOWLIST=true npm run test:integration-metrics-redaction || make cleanup-failure
	README_DENYLIST=true docker-compose up --build --detach integration_dotnet_metrics_v6.0
	sleep 5
